...
type
  TfFrameReportProductJH = class(TfFrameBase)
    PanelR: TPanel;
    PanelL: TPanel;
    LabelHint: TLabel;
...
class function TfFrameReportProductJH.FrameID: integer;
begin
  Result := cFI_FrameReportJH;
end;

procedure TfFrameReportProductJH.OnCreateFrame;
var nIni: TIniFile;
begin
...
procedure TfFrameReportProductJH.OnDestroyFrame;
var nIni: TIniFile;
begin
  nIni := TIniFile.Create(gPath + sFormConfig);
  try
...
procedure TfFrameReportProductJH.BtnExitClick(Sender: TObject);
begin
  Close;
end;

//Desc: 清理数据
procedure TfFrameReportProductJH.ClearData(const nFree: Boolean);
var nIdx: Integer;
begin
...
//Desc: 时间变动
procedure TfFrameReportProductJH.EditTimePropertiesEditValueChanged(Sender: TObject);
var nS,nE: TDate;
begin
...
procedure TfFrameReportProductJH.BtnSearchClick(Sender: TObject);
begin
...
//------------------------------------------------------------------------------
//Desc: 载入产品列表
procedure TfFrameReportProductJH.LoadProductData;
...
begin
  nStr := 'Select Sum(D_Number) As D_InNum,pt.StyleID,StyleName,ColorName,' +
          'SizeName From $DL dl ' +
          ' Left Join $OD od On od.O_ID=dl.D_Order ' +
...
//Desc: 载入产品列表到界面
procedure TfFrameReportProductJH.LoadProductList;
...
//Desc: 统计指定款式的库存总量
function TfFrameReportProductJH.CountNumber(const nID: string): Integer;
var nIdx: Integer;
begin
  Result := 0;
...
//Desc: 查看详情
procedure TfFrameReportProductJH.OnBtnClick(Sender: TObject);
var nStr,nID: string;
...
begin
  ...
  LabelHint.Caption := FPainter.Data[nNum][1].FText + '进货详情';
...
initialization
  gControlManager.RegCtrl(TfFrameReportProductJH, TfFrameReportProductJH.FrameID);
end.
